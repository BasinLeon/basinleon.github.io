...
const Dashboard: React.FC<DashboardProps> = ({ setMode }) => {
    ...
    const uniqueCompanies = new Set(LEADS_DATA.map(r => r.Company)).size;
    const highPriority = LEADS_DATA.filter((r: any) => parseInt(r['GTM Score']) >= 80).length;
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            setTimeLeft(`${days} DAYS TO BREACH`);
        }, 1000);
        return () => clearInterval(interval);
    }, []);

    const MetricCard = ({ label, value, color }: any) => (
        <div className="glass-panel p-6 rounded-2xl border border-slate-800 bg-slate-900/40 hover:border-slate-700 transition-all group cursor-default border-l-4 border-l-gold-500/20">
            <div className="text-[10px] uppercase font-black tracking-widest text-slate-500 mb-2 group-hover:text-slate-400 font-mono">{label}</div>
            <div className={`text-4xl font-mono mb-1 ${color} font-bold tracking-tighter`}>{value}</div>
            <div className="h-1 w-12 bg-slate-800 rounded-full mt-2 overflow-hidden">
                <div className={`h-full ${color.replace('text-', 'bg-')} opacity-30`} style={{width: '60%'}}></div>
            </div>
        </div>
    );
            ...
            <div className="flex justify-between items-center mb-10">
                <div>
                    <h1 className="text-3xl font-black text-white uppercase tracking-tighter mb-1">
                        Basin<span className="text-[#D4AF37]">::</span>Nexus <span className="text-xs align-top bg-[#D4AF37] text-black px-1 rounded ml-1">Sovereign V6</span>
                    </h1>
                    <div className="text-xs text-slate-500 font-mono uppercase tracking-[0.3em]">Revenue Architect // Central Command</div>
                </div>
                <div className="text-right">
                    <div className="text-2xl font-mono text-red-500 font-bold animate-pulse">{timeLeft}</div>
                    <div className="text-[10px] text-red-900 uppercase font-black tracking-[0.3em]">Strategic Deadline</div>
                </div>
            </div>

            {/* Metrics Grid */}
            <div className="grid grid-cols-4 gap-6 mb-10">
                <MetricCard label="Total Signals" value={totalLeads.toLocaleString()} color="text-gold-400" />
                <MetricCard label="Enriched Orgs" value={uniqueCompanies.toLocaleString()} color="text-cyan-400" />
                <MetricCard label="Attack Ready" value={highPriority.toLocaleString()} color="text-red-500" />
                <MetricCard label="Refinery Status" value="ACTIVE" color="text-emerald-400" />
            </div>
